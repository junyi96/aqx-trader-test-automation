...
Tests for creating limit orders with various expiry types
"""
import pytest
...
class TestLimitOrders:
    """Test cases for limit orders with different expiry types"""
    ...
    def test_create_limit_order_with_expiry(
        ...
        """
        Test creating limit orders with different expiry types

        This is a data-driven test that runs for each expiry type
        ...
        logger.info(f"Limit order created successfully with {expiry_type}")
        ...
        assert len(pending_orders) > 0, "No pending orders found after creating limit order"

        logger.info(f"Found {len(pending_orders)} pending order(s)")

    def test_limit_order_good_till_canceled(
        self,
        trading_page: TradingPage,
        assets_page: AssetsPage
    ):
        """Test creating a limit order with Good Till Canceled expiry"""
        logger.info("Testing Limit Order - Good Till Canceled")
        ...
        trading_page.place_limit_order(**order_data)
        logger.info("Limit order (GTC) created successfully")

    def test_limit_order_good_till_day(
        self,
        trading_page: TradingPage,
        assets_page: AssetsPage
    ):
        """Test creating a limit order with Good Till Day expiry"""
        logger.info("Testing Limit Order - Good Till Day")
        ...
        trading_page.place_limit_order(**order_data)
        logger.info("Limit order (GTD) created successfully")

    def test_limit_order_good_till_date(
        self,
        trading_page: TradingPage,
        assets_page: AssetsPage
    ):
        ...
        trading_page.place_limit_order(**order_data)
        logger.info(f"Limit order (GTDate) created with expiry: {future_date}")

    def test_limit_order_good_till_date_and_time(
        self,
        trading_page: TradingPage,
        assets_page: AssetsPage
    ):
        ...
        trading_page.place_limit_order(**order_data)
        logger.info(f"Limit order (GTDateTime) created with expiry: {future_datetime}")